...
if __name__ == '__main__':
    ...
    # bootstrap_servers: consumer contact with these servers at first, then broker will tell consumer the data on
    # which server in cassandra called contact points in industry, don't contact with ip and port directly,
    # using url like kafka.bootstrap.bittiger.com to contact there are many servers inside this url, some down,
    # can contact with others
    ...
    session.execute(
        "CREATE TABLE IF NOT EXISTS %s (stock_symbol text, trade_time timestamp, trade_price float, PRIMARY KEY ("
        "stock_symbol, trade_time))" % data_table)
